"""Train a committor function for chignolin (1UAO) protein folding.

This script implements the full feedback loop:
1. Load or build the OpenMM system from setup_system.py output
2. Initialize a committor model (MACE foundation model or DistanceEncoder)
...
``system.xml`` is built from ``setup_system.py``; ``pdb_folded`` / ``pdb_unfolded``
must have the same atom count as that system.
"""
...
def _openmm_missing_message() -> str:
    base = (
        ...
        "Install OpenMM in the same environment, e.g.:\n"
...
def _assert_pdb_matches_system(
    ...
    if n_sys != n_pdb:
        raise ValueError(
            ...
            "Use PDBs from the same setup_system.py run."
        )
...
def _require_input_paths(
    ...
    if not Path(pdb_unfolded).is_file():
        msg += (
            "\nTo create the unfolded structure, run:\n"
            "  python make_unfolded_snapshot.py --input_pdb output/minimized.pdb \\\n"
            "      --system_xml output/system.xml --output output/unfolded.pdb"
        )
...
def _remove_barostat_for_nvt(simulation_system: "openmm.System") -> None:
    ...
    for i in range(n - 1, -1, -1):
        if isinstance(simulation_system.getForce(i), openmm.MonteCarloBarostat):
            ...
            removed += 1
    if removed:
        logger.info("Removed %d MonteCarloBarostat force(s); using NVT.", removed)
...
def train(
    ...
    """Run the committor training loop for chignolin.
    ...
    logger.info("Starting training: %d iterations", n_iterations)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train committor for chignolin (1UAO) protein folding"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--pdb_folded",
        required=True,
        help="Folded state PDB (same atoms as system.xml)",
    )
    parser.add_argument(
        "--pdb_unfolded",
        required=True,
        help="Unfolded state PDB (same atoms as system.xml)",
    )
    parser.add_argument(
        "--reference_pdb",
        default=None,
    ...
    parser.add_argument(
        "--method", default="opes", choices=["opes", "umbrella"]
    )
    ...
    parser.add_argument(
        "--platform", default="CPU", choices=["CPU", "CUDA", "OpenCL"]
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="Hidden dimension for DistanceEncoder MLP (test-only).",
    )
