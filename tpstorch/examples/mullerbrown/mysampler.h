...
class MySampler : public MLSamplerEXP
{
    public:
        ...
        */ 
        ...
        void acceptRejectEnergyWell(const torch::Tensor& state, const double& energy_ref)//, const double& committor_val, bool onlytst = false, bool bias = false)
        {
            float* state_sys = state.data_ptr<float>();
            system->MCStepEnergyWell(state_sys, energy_ref);
        };
